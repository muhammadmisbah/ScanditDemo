...
import { AppState, BackHandler, Dimensions, SafeAreaView, TouchableOpacity, Text } from 'react-native';
import {
  BarcodeTracking,
...
export class App extends Component {
  ...
  setupScanning() {
    ...
    this.feedback = Feedback.defaultFeedback;
        ...
        Object.values(session.trackedBarcodes).forEach((trackedBarcode) => {
          this.viewRef.current.viewQuadrilateralForFrameQuadrilateral(trackedBarcode.location)
            .then((location) => {
              ...
              this.updateView(trackedBarcode, location)
            });
        });
      },
    };
  ...
  render() {
    return (
      <>
        ...
        <TouchableOpacity onPress={this.toggleScan} style={{ backgroundColor: "rgba(255,255,255,0.5)", position: "absolute", top: 20, right: 20, padding: 20 }}>
          <Text>{this.state.scanning ? "Close" : "Open"}</Text>
        </TouchableOpacity>
      </>
    );
  }
